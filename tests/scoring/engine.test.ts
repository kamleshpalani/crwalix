...
import { BusinessStatus, PriorityTier, WebsiteHealth, WebsiteStatus } from '@crawlix/shared';
...
describe('scoring engine', () => {
  ...
  it('demotes a fresh-website lead to LOW', () => {
    const lead = baseLead({
      websiteStatus: WebsiteStatus.EXISTS,
      websiteHealth: WebsiteHealth.FRESH
    });
    const r = score(lead, defaultRuleset);
    expect(r.priorityTier).toBe(PriorityTier.LOW);
  });

  it('punishes permanently closed leads', () => {
    const closed = score(baseLead({ businessStatus: BusinessStatus.CLOSED_PERM }), defaultRuleset);
    const open = score(baseLead({ businessStatus: BusinessStatus.OPERATIONAL }), defaultRuleset);
    expect(closed.score).toBeLessThan(open.score);
  });

  it('returns rule breakdown', () => {
    const r = score(baseLead({ websiteStatus: WebsiteStatus.HIGH_CONFIDENCE_NONE }), defaultRuleset);
