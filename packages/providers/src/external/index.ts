...
export const pageSpeedProvider = {
  ...
  async run(url: string): Promise<PageSpeedResult | null> {
    ...
    const pct = (s?: number): number | null => (typeof s === 'number' ? Math.round(s * 100) : null);
    const lcp = audits['largest-contentful-paint']?.numericValue;
    const cls = audits['cumulative-layout-shift']?.numericValue;
    const inp = audits['interactive']?.numericValue ?? audits['total-blocking-time']?.numericValue;
    return {
      performance: pct(cats['performance']?.score),
      accessibility: pct(cats['accessibility']?.score),
      bestPractices: pct(cats['best-practices']?.score),
      seo: pct(cats['seo']?.score),
      lcpSeconds: typeof lcp === 'number' ? Math.round((lcp / 1000) * 10) / 10 : null,
  ...
  async run(domain: string): Promise<BuiltWithResult | null> {
    ...
    for (const g of data.groups ?? []) {
      for (const c of g.categories ?? []) {
        ...
        techs.push(tag);
        if (!cms && /wordpress|shopify|wix|squarespace|webflow|drupal|joomla|magento|ghost/.test(tag)) cms = tag;
        if (!ecommerce && /shopify|woocommerce|magento|bigcommerce|prestashop/.test(tag)) ecommerce = tag;
        if (/analytics|pixel|tag-manager|hotjar|mixpanel/.test(tag)) analytics.push(tag);
      }
    }
    return {
...
export const hunterProvider = {
  ...
  async run(domain: string): Promise<DomainContactsResult | null> {
    ...
    const data = json as {
      data?: { emails?: { value: string; first_name?: string; last_name?: string; position?: string; linkedin?: string; confidence?: number }[] };
    };
    ...
    const decisionMaker = emails.find((e) => /owner|ceo|founder|director|manager/i.test(e.position ?? '')) ?? emails[0];
    return {
      emails,
      primaryEmail: emails[0]?.email ?? null,
      decisionMakerName: decisionMaker
        ? [decisionMaker.firstName, decisionMaker.lastName].filter(Boolean).join(' ').trim() || null
        : null,
      fetchedAt: new Date().toISOString()
    };
  }
...
export const apolloProvider = {
  ...
  async run(domain: string): Promise<DomainContactsResult | null> {
    ...
    const data = json as { people?: { email?: string; first_name?: string; last_name?: string; title?: string; linkedin_url?: string }[] };
    ...
    return {
      ...
      decisionMakerName: emails[0]
        ? [emails[0].firstName, emails[0].lastName].filter(Boolean).join(' ').trim() || null
        : null,
      fetchedAt: new Date().toISOString()
    };
  }
...
export const brightLocalProvider = {
  ...
  async run(_args: { businessName: string; city?: string; country?: string }): Promise<BrightLocalRankResult | null> {
...
export function listExternalProviders(): ExternalProviderStatus[] {
  return [
    { id: 'pagespeed', name: 'PageSpeed Insights', envVars: ['PAGESPEED_API_KEY', 'GOOGLE_API_KEY'], configured: pageSpeedProvider.available(), category: 'speed' },
    { id: 'builtwith', name: 'BuiltWith', envVars: ['BUILTWITH_API_KEY'], configured: builtWithProvider.available(), category: 'tech' },
    { id: 'hunter', name: 'Hunter', envVars: ['HUNTER_API_KEY'], configured: hunterProvider.available(), category: 'email' },
    { id: 'apollo', name: 'Apollo', envVars: ['APOLLO_API_KEY'], configured: apolloProvider.available(), category: 'email' },
    { id: 'zerobounce', name: 'ZeroBounce', envVars: ['ZEROBOUNCE_API_KEY'], configured: zeroBounceProvider.available(), category: 'verify' },
    { id: 'neverbounce', name: 'NeverBounce', envVars: ['NEVERBOUNCE_API_KEY'], configured: neverBounceProvider.available(), category: 'verify' },
    { id: 'screenshotone', name: 'ScreenshotOne', envVars: ['SCREENSHOTONE_ACCESS_KEY'], configured: screenshotOneProvider.available(), category: 'screenshot' },
    { id: 'brightlocal', name: 'BrightLocal', envVars: ['BRIGHTLOCAL_API_KEY'], configured: brightLocalProvider.available(), category: 'rank' }
  ];
}
