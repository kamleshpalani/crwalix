...
const missingWebsite: ScoreRule = {
  ...
  evaluate: (l) => {
    ...
    const isSocialOnly = host !== null && SOCIAL_ONLY_HOSTS.some((d) => host === d || host.endsWith(`.${d}`));
    if (isSocialOnly) {
      return { contribution: 35, reason: `Only a social/booking link (${host}) — no real website` };
    }
    switch (l.websiteStatus) {
      case WebsiteStatus.HIGH_CONFIDENCE_NONE:
        return { contribution: 40, reason: 'Confirmed no website — prime prospect' };
  ...
  evaluate: (l) => {
    switch (l.websiteHealth) {
      ...
      case WebsiteHealth.OUTDATED:
        return { contribution: 25, reason: 'Outdated site — strong redesign candidate' };
      case WebsiteHealth.NEEDS_REVIEW:
        return { contribution: 15, reason: 'Site has aging / weak-conversion signals' };
...
const reviewActivity: ScoreRule = {
  ...
  evaluate: (l) => {
    ...
    if (c >= 50) return { contribution: 15, reason: `Healthy review volume (${c})` };
...
const ratingRule: ScoreRule = {
  ...
  evaluate: (l) => {
    const r = l.rating ?? 0;
    if (r >= 4.5) return { contribution: 15, reason: `Excellent rating (${r})` };
...
const highValueCategory: ScoreRule = {
  ...
  evaluate: (l) => {
    ...
    if (HIGH_VALUE_CATEGORY.test(haystack)) {
      return { contribution: 20, reason: 'High-value SME category (likely to invest in website)' };
    }
...
const contactReachability: ScoreRule = {
  ...
  evaluate: (l) => {
    if (l.phone && l.phone.trim().length >= 6) {
      return { contribution: 10, reason: 'Phone number available for outreach' };
    }
